...
// FuncPermissionsResult http 没有功能权限错误返回
func FuncPermissionsResult(r *http.Request, w http.ResponseWriter, err error) {
	errMsg := fmt.Sprintf("%s ,%s", xerr.MapErrMsg(xerr.MISSED_FUNC_PERMISSIONS_ERROR), err.Error())
	httpx.WriteJson(w, http.StatusOK, Error(xerr.MISSED_FUNC_PERMISSIONS_ERROR, errMsg))
}

// 需要登录提示
func NotLogin(r *http.Request, w http.ResponseWriter) {
	errMsg := fmt.Sprintf("%s", xerr.MapErrMsg(xerr.LOGIN_EXPIRE_ERROR))
	httpx.WriteJson(w, http.StatusNonAuthoritativeInfo, Error(xerr.LOGIN_EXPIRE_ERROR, errMsg))
}
